...
def createPharmacist(request):
    ...
    if request.method == "POST":       
        ...
        try:
            ...
            messages.error(request, "Failed to Add Pharmacist!")
            return redirect('add_pharmacist')
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            # First Update into Custom User Model
                ...
                messages.success(request, "Failed! Details Not Updated.")
                return redirect('all_patients')
...
def deleteDoctor(request,pk):
    try:
        ...
        if request.method == 'POST':
            doctor.delete()
            messages.success(request, "Doctor Details Deleted")
            return redirect('manage_doctor')
    except:
        messages.error(request, "Failed! Doctor Aready Deleted")
        return redirect('manage_doctor')
...
def deletePharmacist(request,pk):
    try:
        ...
        if request.method == 'POST':
            pharmacist.delete()
            messages.success(request, "Pharmacist Details Deleted")
            return redirect('manage_pharmacist')
    except:
        messages.error(request, "Failed! Pharmacist Aready Deleted")
        return redirect('manage_pharmacist')
    return render(request,'hod_templates/sure_delete.html')
...
def deletePharmacyClerk(request,pk):
    try:
        ...
        if request.method == 'POST':
            clerk.delete()
            messages.success(request, "Clerk Details Deleted")
            return redirect('manage_pharmacyClerk')
    except:
        messages.error(request, "Failed! Clerk Already Deleted")
        return redirect('manage_pharmacyClerk')
    return render(request,'hod_templates/sure_delete.html')
...
def editPharmacyClerk(request,clerk_id):
    ...
    if request.method == "POST":
        ...
        try:
            ...
            clerk.gender=gender
            clerk.save()
            messages.success(request,'Clerk Details Updated')
            return redirect('manage_pharmacyClerk')
        except:
    ...
    if request.method == "POST":
        if form.is_valid():
            ...
            try:
                ...
                drugs.save()
                form.save()
                messages.success(request,'Drug Updated Succefully')
                return redirect('manage_stock')
            except:
                messages.error(request,'An Error Was Encounterd Drug Not Updated')
                return redirect('manage_stock')
...
def deleteDrug(request,pk):
    try:
        ...
        messages.error(request, "Drug Aready Deleted")
        return redirect('manage_stock')
    return render(request,'hod_templates/sure_delete.html')
